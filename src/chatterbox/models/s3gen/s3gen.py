...
class S3Token2Wav(S3Token2Mel):
    ...
    def inference_chunk(
        ...
        """
        Process a chunk of speech tokens for streaming audio generation.
        
        Args:
            speech_tokens: Token chunk to convert to audio.
            ref_dict: Pre-computed reference embeddings from embed_ref().
            cache_source: HiFi-GAN source cache from previous chunk (for continuity).
            is_first_chunk: Whether this is the first chunk (apply initial fade-in).
            is_last_chunk: Whether this is the last chunk (finalize flow).
            crossfade_samples: Number of samples to crossfade between chunks.
            
        Returns:
            Tuple of (audio_chunk, new_cache_source) where:
                - audio_chunk: Generated audio for this chunk
                - new_cache_source: Cache to pass to next inference_chunk call
        """
        if cache_source is None:
            cache_source = torch.zeros(1, 1, 0).to(self.device)
        ...
        # Convert mel to waveform with source continuity
        output_wavs, new_cache_source = self.hift_inference(output_mels, cache_source)
        
        # Apply fade-in only on first chunk
        if is_first_chunk:
            fade_len = min(output_wavs.size(1), len(self.trim_fade))
            output_wavs[:, :fade_len] *= self.trim_fade[:fade_len]
        
        return output_wavs, new_cache_source
